...
for host in range(1, k * k * k / 4 + 1):
  ...
  print '$ns at %f \"$f%s dump-link-stat %s\"' % (largest + 1000, host, edge)
  print '$ns at %f \"$f%s dump-link-stat %s\"' % (largest + 1000, edge, host)
...
for edge in range(k * k * k / 4 + 1, k * k * k / 4 + k * k / 2 + 1):
  ...
  for aggr in range(aggr_r1, aggr_r2 + 1):
    print '$ns at %f \"$f%s dump-link-stat %s\"' % (largest + 1000, edge, aggr)
    print '$ns at %f \"$f%s dump-link-stat %s\"' % (largest + 1000, aggr, edge)
...
for aggr in range(k * k * k / 4 + k * k / 2 + 1, k * k * k / 4 + k * k + 1):
  ...
  for core in range(core_r1, core_r2 + 1):
    print '$ns at %f \"$f%s dump-link-stat %s\"' % (largest + 1000, aggr, core)
    print '$ns at %f \"$f%s dump-link-stat %s\"' % (largest + 1000, core, aggr)
'''
